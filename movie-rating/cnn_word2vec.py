...
def evaluate_lenet5(learning_rate=0.01, n_epochs=200,
		    ...
		    num_features=300,dim=100,
                    nkerns=[40, 40], batch_size=100):
    """ 
...
#    dim=100
    ...
    #theano variable
    train_set_x = theano.shared(value=trainFeatureVecs,borrow=True) #20000, 1, 50, 300
    ...
    valid_set_x = theano.shared(value=validFeatureVecs,borrow=True) #5000, 1, 50, 300
    ...
    # filtering reduces the image size to (100-5+1 , 300-300+1) = (86, 271)
    # maxpooling reduces this further to (86/5, 271/10) = (17, 27)
    # 4D output tensor is thus of shape (batch_size, nkerns[0], 9, 1)
    layer0 = LeNetConvPoolLayer(
        rng,
        input=layer0_input,
        image_shape=(batch_size, 1, dim, num_features),
        filter_shape=(nkerns[0], 1, 10, num_features),
        poolsize=(9,1)
    )

    l1feature_r=dim-10+1#568-10+1=559
    l1feature_c=num_features-300+1
    l1feature_r=l1feature_r/10
    l1feature_c=l1feature_c

    """ 
    # Construct the first convolutional pooling layer:
    # filtering reduces the image size to (17-5+1 , 27-5+1) = (12, 23)
    # maxpooling reduces this further to (6/3, 16/4) = (2, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 2, 4)
    layer1 = LeNetConvPoolLayer(
        rng,
        input=layer0.output,
        image_shape=(batch_size, nkerns[0], 17, 27),
    ...
    layer2 = HiddenLayer(
        rng,
        input=layer2_input,
        n_in=nkerns[0] *l1feature_r*l1feature_c,
        n_out=500,
        activation=T.tanh
    )
